 ...
 * Time Complexity:
 * insertionSort:
 *   最佳 O(n)：當陣列已排序時，內層 while 不執行任何移動。
 *   最壞 O(n^2)：當陣列為反序，key 每次都需移動到最前面，總共進行約 n(n-1)/2 次比較與移動。
 *   平均 O(n^2)：隨機排列情況下，也需約 n²/4 次操作。

 * binaryInsertionSort:
 *   最佳 O(n log n)：每次使用二分搜尋找插入點需 O(log n)，進行 n 次。
 *   最壞 O(n^2)：雖然找插入點是 O(log n)，但移動元素仍可能需 O(n)。
 *   平均 O(n^2)：雖然比較次數減少，但移動次數沒變，因此整體仍為 O(n²)。
 */
